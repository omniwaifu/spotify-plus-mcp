...
def validate(func: Callable[..., T]) -> Callable[..., T]:
    ...
    def wrapper(self, *args, **kwargs):
        # Handle authentication - check if token needs refresh
        if hasattr(self, '_is_token_expired') and self._is_token_expired():
            if hasattr(self, '_refresh_token') and self.config.get("refresh_token"):
                if not self._refresh_token():
                    raise ValueError(
                        "Authentication failed. Please use the Authentication tool with action 'get_auth_url' "
                        "to start the OAuth flow, then use 'exchange_code' with the authorization code."
                    )
                # Reinitialize client with new token
                self._init_spotify_client()
        elif not self.auth_ok():
            # Try old auth_refresh method if it exists
            if hasattr(self, 'auth_refresh'):
                try:
                    self.auth_refresh()
                ...
                raise ValueError(
                    "Not authenticated. Please use the Authentication tool with action 'get_auth_url' "
                    "to start the OAuth flow, then use 'exchange_code' with the authorization code."
                )
